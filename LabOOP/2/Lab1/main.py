...
def comparator_alfabetic(s1:StudentCNP, s2:StudentCNP) -> bool:
    return s1.nume_prenume > s2.nume_prenume
...
class Stiva:
    ...
    def sorteaza(self, comp:Callable[[StudentCNP, StudentCNP], bool]) -> None:
        sortat = False
        while not sortat:
            sortat = True
            for i in range(self.n - 1):
                if comp(self[i], self[i + 1]):
                    self[i], self[i + 1] = self[i + 1], self[i]
                    sortat = False

    def partitie(self, s: int, d: int, comp:Callable[[StudentCNP, StudentCNP], bool]) -> int:
        pivot = self[d]
        poz = s
        i = s
        while i < d:
    ...
    def sorteazaQ(self, s: int, d: int, comp:Callable[[StudentCNP, StudentCNP], bool]):
        if s < d:
            poz = self.partitie(s, d, comp)
            self.sorteazaQ(s, poz - 1, comp)
            self.sorteazaQ(poz + 1, d, comp)

    def sorteaza_alfabetic(self) -> None:
        self.sorteaza(comparator_alfabetic)

    def sorteaza_medie(self) -> None:
        self.sorteaza(comparator_medie)

    def __contains__(self, student:StudentCNP) -> bool:
    ...
    def __add__(self, other:Stiva)->Stiva:
...
def main():
    ...
    an = g1 + g2  # an va conține toți studenții din g1 și g2

    e = Stiva("exmatriculati.txt")

    an = an - e  # elimină studenții exmatriculați din an
