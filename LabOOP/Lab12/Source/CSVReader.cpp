...
Student* CSVReader::prelucreaza_linie(char* line) {
    try {
        ...
        char* cnp = strtok(nullptr, ",");

        Student* student_nou;
        if (cnp == nullptr)
            student_nou = new Student(string(nume)+" "+prenume, specializare, an, medie);
        else student_nou = new StudentCNP(string(nume)+" "+prenume, specializare, an, medie, cnp);

        return student_nou; // 'nume' e convertit la string deoarece astfel putem concatena mai usor cu operatorul '+'
    }
